...
export function evaluateAction(action: ActionRequest, evidence: EvidenceItem[], trust: AgentTrust, resolved = false): ActionDecision {
  ...
  if (action.scenarioId === "human-refund") {
    return {
      authorization: "APPROVAL_REQUIRED",
      recommendation: "APPROVE",
      confidence,
      policy: "PASS",
      authority: `${getAutonomyLabel(trust.autonomy)} · refund approval threshold`,
      reason: "The evidence supports this refund, but policy requires human authorization for amounts above $500.",
    };
  }
  ...
  return {
    ...
    confidence,
    policy: "PASS",
    authority: `${getAutonomyLabel(trust.autonomy)} · autonomous`,
