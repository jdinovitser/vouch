...
const getSession = (req: express.Request, res: express.Response) => {
  ...
  if (!id || !sessions.has(id)) {
    ...
    sessions.set(id, {
      ...
      service: { mode: process.env.VOUCH_ENABLE_AWS === "true" ? "AWS" : "DEMO", available: true, message: process.env.VOUCH_ENABLE_AWS === "true" ? "Agent service connected" : "Demo mode active" },
    });
  }
...
function addAudit(session: SessionState, record: ActionRecord, type: string, actor: "VOUCH" | "HUMAN", status: string, result: string) {
  session.audit.unshift(auditFor(record, type, actor, status, result));
}
...
app.post("/api/scenarios/:scenarioId/run", async (req, res) => {
  ...
  const record: ActionRecord = { action: scenario.action, state, createdAt: new Date().toISOString() };
  ...
  const conflict = (scenario.hasConflict && !resolved) || scenario.hasInjection;
  if (conflict) state = transition(state, "CONFLICT_DETECTED");
  state = transition(state, conflict ? "BLOCKED" : "RISK_ASSESSED");
  if (conflict) {
    const agentResult = await runStrandsEvaluation(scenario.action, scenario.evidence, session.trust, resolved);
...
app.post("/api/actions/:actionId/resolve", (req, res) => {
  const session = getSession(req, res);
  if (session.activeScenarioId !== "conflicting-refund") return res.status(409).json({ error: "No evidence conflict is awaiting resolution" });
  session.activity = ["Human resolution received", "New authoritative approval attached", "Re-evaluating evidence…"];
  return res.json({ session, scenarios, message: "Authoritative approval added. Run the action again." });
});
