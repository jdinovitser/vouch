...
const caseTemplates: Array<Omit<ClaimsCase, "status" | "receivedAt">> = [
  ...
  { id: "case-1192", scenarioId: "verification-failure", caseNumber: "#1192", customer: "Priya Shah", category: "Account review", summary: "Review is complete, but the resulting account state must be verified after update.", priority: "MEDIUM" },
...
export function initialCases(): ClaimsCase[] {
  ...
  return caseTemplates.map((item) => ({ ...item, status: "NEW", receivedAt }));
}
