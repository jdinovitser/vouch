...
function Landing({ navigate, trust }: { navigate: (v: View) => void; trust?: AgentTrust }) {
  return <div className="landing">
    <section className="landing-hero content-width">
      <div className="hero-copy"><div className="eyebrow"><span className="eyebrow-line" />A TRUSTED AI TEAMMATE FOR GROUPS HELPING THEIR COMMUNITIES</div><h1>The AI that<br /><em>looks out for people.</em></h1><p className="hero-lead">VOUCH lets your team delegate routine work to an AI teammate while keeping people in control of the decisions that matter.</p><div className="earned-loop"><b>HELP</b><i /><b>PROVE</b><i /><b>VERIFY</b><i /><b>EARN</b></div><div className="hero-actions"><button className="button primary" onClick={() => navigate("demo")}><Zap size={16} />See VOUCH in action <ArrowRight size={16} /></button><button className="button ghost" onClick={() => navigate("how")}>How it works <ChevronRight size={16} /></button></div><div className="hero-proof"><span><ShieldCheck size={16} /> The agent starts small</span><span><Users size={16} /> The group stays in charge</span><span><Fingerprint size={16} /> Results are verified</span></div></div>
    ...
    <section className="trust-manifesto content-width"><div><span className="section-kicker">VOUCH IS THE BACKUP</span><h2>When AI isn’t sure,<br /><span>VOUCH has your back.</span></h2><p>The goal is not to stop AI from helping. It is to let AI handle routine work while bringing people back into the loop when something actually matters.</p></div><button className="text-link" onClick={() => navigate("demo")}>See the human decision <ArrowRight size={17} /></button></section>
  </div>;
}
...
function TeamDelegation() {
  const steps = [
    ...
    ["PROVE IT", "The agent handles real work for the group."],
    ["VERIFY", "The group can see whether the work actually succeeded."],
    ["EARN TRUST", "Successful verified work can earn more responsibility."],
    ["STEP IN", "When something is too risky, the decision returns to a person."],
  ];
  return <section className="team-delegation content-width"><div className="delegation-copy"><div className="section-kicker">EVERYONE DESERVES A LITTLE BACKUP</div><h2>Your team shouldn&apos;t have to<br /><em>do everything itself.</em></h2><p>A few people can only do so much. An AI teammate can take care of repetitive work—but when that teammate acts on behalf of the group, someone still needs to know what it is allowed to do.</p><strong>VOUCH lets your group delegate safely.</strong></div><div className="delegation-steps">{steps.map(([title, text], index) => <div key={title}><span>0{index + 1}</span><b>{title}</b><p>{text}</p>{index < steps.length - 1 && <ArrowDown size={15} />}</div>)}</div></section>;
}

function CommunityCapacity({ navigate }: { navigate: (v: View) => void }) {
  const groups = [
    ["NEIGHBORHOOD GROUPS", "Share the repetitive load.", "Coordinate routine requests without putting every decision on one volunteer."],
    ["FOOD PANTRY TEAMS", "Give volunteers more capacity.", "Let an AI teammate help while keeping important decisions with people."],
    ["SCHOOLS + LIBRARIES", "Handle routine work without overstepping.", "Give the team useful help with a clear boundary for human judgment."],
    ["SMALL LOCAL ORGANIZATIONS", "Get another set of hands.", "Delegate routine work without giving up control of the group."],
  ];
  return <section className="community-capacity content-width"><div className="community-heading"><div className="section-kicker">A GOOD NEIGHBOR AGENT</div><h2>An AI teammate for<br /><em>the people doing the work.</em></h2><p>Imagine a small group with more requests than volunteers. They do not want to approve every task, and they do not want an AI deciding its own power. VOUCH gives the group a middle ground.</p><button className="text-link" onClick={() => navigate("how")}>See how trust is earned <ArrowRight size={17} /></button><small>Illustrative use cases—not current customer or deployment claims.</small></div><div className="community-grid">{groups.map(([title, headline, text], index) => <article key={title}><span>0{index + 1}</span><b>{title}</b><h3>{headline}</h3><p>{text}</p></article>)}</div></section>;
}

function TeammateRoles() {
  return <section className="teammate-roles content-width"><div className="section-kicker">WHO DOES WHAT</div><h2>The agent is the teammate.<br /><em>The group is the user.</em></h2><p className="roles-intro">VOUCH does not replace the people running the work. It helps them delegate.</p><div className="roles-grid"><article><Bot size={24} /><span>THE AGENT</span><h3>Helps with the work.</h3><p>Investigates, reasons, recommends, and handles routine tasks.</p></article><article className="vouch-role"><ShieldCheck size={24} /><span>VOUCH</span><h3>Checks the boundary.</h3><p>Evaluates evidence, risk, authority, verification, and safety.</p></article><article><Users size={24} /><span>THE GROUP</span><h3>Keeps the keys.</h3><p>Sets the boundaries and makes decisions that require human judgment.</p></article></div><strong>The AI can help. The people keep the keys.</strong></section>;
}

function HumanControl({ navigate }: { navigate: (v: View) => void }) {
  return <section className="human-control content-width"><div><div className="section-kicker">WHEN THE DECISION MATTERS</div><h2>Bring people<br /><em>back in.</em></h2><p>VOUCH is not designed to keep humans approving every action. It is designed to know when a human should decide.</p><button className="text-link" onClick={() => navigate("demo")}>See the human decision <ArrowRight size={17} /></button></div><div className="control-paths"><article className="allow"><Check size={18} /><span>ROUTINE + VERIFIED</span><b>Agent can act</b></article><article className="approval"><Users size={18} /><span>HIGHER RISK</span><b>Human approval</b></article><article className="block"><ShieldAlert size={18} /><span>UNSAFE OR FORBIDDEN</span><b>VOUCH blocks</b></article></div><strong>The goal is useful autonomy that a group can trust—not maximum autonomy.</strong></section>;
}

function WhyVouch({ navigate }: { navigate: (v: View) => void }) {
  return <div className="story-page why-page content-width">
    <StoryHero kicker="WHY VOUCH" title={<>Your AI teammate shouldn&apos;t<br /><em>become your boss.</em></>} text="When a group delegates work to an AI, it needs more than an agent that can act. It needs confidence that the agent will stay within the responsibility the group gave it." mascot="blocked" heroArtwork="warrior" cta="See VOUCH in action" onClick={() => navigate("demo")} />
    <section className="story-section"><div className="section-kicker">PEOPLE KEEP THE KEYS</div><h2>Get more help from AI<br /><em>without giving AI control of the group.</em></h2><p className="story-lead">People helping people should not have to choose between doing everything themselves and handing an AI the keys.</p><div className="option-grid"><Option title="BOUNDARIES" tag="The agent cannot promote itself." text="Responsibility comes from the group and the independent VOUCH authority layer." tone="muted" /><Option title="EARNED RESPONSIBILITY" tag="Trust follows proof." text="Successful verified work can increase what the teammate is trusted to handle." tone="success" /><Option title="HUMAN DECISIONS" tag="Important choices come back." text="Higher-risk actions return to the people instead of becoming automatic." tone="success" /><Option title="RECOVERY + SAFETY" tag="Authority can decrease." text="Failed verification reduces responsibility, and hard boundaries block unsafe work." tone="danger" /></div></section>
    <section className="value-section"><div><div className="section-kicker">THE HUMAN QUESTIONS</div><h2>Before the teammate acts,<br /><em>VOUCH checks.</em></h2></div><div className="change-list"><Change from="TRUST" to="Can we trust what is happening?" /><Change from="AUTHORITY" to="Is the agent actually allowed to do this?" /><Change from="RISK" to="What could happen if it gets this wrong?" /><Change from="VERIFICATION" to="Did the result actually succeed?" /><Change from="HUMAN APPROVAL" to="Is this important enough for someone to decide?" /></div></section>
    <CommunityCapacity navigate={navigate} />
    <RoiSection />
    <section className="why-now"><div className="section-kicker">WHEN AI CAN ACT</div><h2>Let AI handle the busywork.<br /><em>Let people decide what matters.</em></h2><div className="automation-ladder"><span>The group delegates <b>limited, routine responsibility</b></span><ArrowRight /><span>The teammate proves itself <b>results are independently verified</b></span><ArrowRight /><span className="current">VOUCH adjusts trust <b>the group remains in charge</b></span></div></section>
    <PageCta title="The AI can help. The people keep the keys." text="See how a teammate earns responsibility without gaining unlimited power." label="See earned delegation" onClick={() => navigate("demo")} />
  </div>;
}
...
function RoiSection() {
  const items = [["RETURN STAFF TIME", "Routine work moves from minutes to seconds."], ["PROTECT IMPORTANT DECISIONS", "Unsupported or risky actions stop before they affect people."], ["FOCUS HUMAN JUDGMENT", "People step in when their decision is actually needed."], ["KEEP ACCOUNTABILITY VISIBLE", "Every important action has a decision and verification record."]];
  return <section className="roi-section"><div className="roi-heading"><div className="section-kicker">COMMUNITY CAPACITY</div><h2>More capacity for the people<br /><em>doing the work.</em></h2><p>VOUCH helps community-serving teams use agentic automation without asking them to trade away human responsibility.</p></div><div className="roi-cards">{items.map(([title, text], index) => <div className="roi-card" key={title}><span>0{index + 1}</span><b>{title}</b><p>{text}</p></div>)}</div><div className="attention-visual"><span>HUMAN ATTENTION</span><div><label>WITHOUT VOUCH</label><i className="attention-before" /></div><div><label>WITH VOUCH</label><i className="attention-after" /></div><p>Reserve human judgment for decisions that actually need it.</p></div></section>;
}
...
function HowItWorks({ navigate }: { navigate: (v: View) => void }) {
  ...
  return <div className="story-page how-page content-width">
    <StoryHero kicker="HOW VOUCH HELPS" title={<>Let AI handle the busywork.<br /><em>Let people decide what matters.</em></>} text="Before an agent takes an important action for a person or community, VOUCH checks the evidence, boundaries, risk, and whether human judgment is needed." mascot="warrior" cta="See VOUCH in action" onClick={() => navigate("demo")} />
...
function DemoExperience({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="demo-experience content-width">
    <div className="demo-hero-heading"><div><div className="eyebrow"><span className="eyebrow-line" />SEE EARNED DELEGATION IN ACTION</div><h1>The agent helps.<br /><em>The group stays in control.</em></h1><p>Claims resolution is our demonstration scenario for showing how a group can safely delegate work to an AI teammate. The claims scenario is the proof—not the identity of the product.</p></div><KnightAuthority level={session.trust.autonomy} size="normal" state="deployment" /></div>
    ...
    <section className="demo-outcome"><div><span className="section-kicker">WHEN THE ANSWER ISN’T CLEAR</span><h2>VOUCH has your back.<br /><em>Five honest answers.</em></h2><p>See routine work move, important decisions return to people, unsafe instructions stop, and every completed outcome get checked.</p></div><div className="scenario-rail">{scenarios.filter((scenario) => !scenario.recovery && scenario.id !== "recovered-account-update").map((scenario) => <button className={`scenario-mini ${scenario.accent} ${scenario.id === scenarioId ? "selected" : ""}`} onClick={() => { setScenarioId(scenario.id); void runScenario(scenario.id); }} disabled={loading} key={scenario.id}><span className="mini-label">{scenario.action.risk} RISK</span><b>{scenario.name}</b><small>{scenario.hasConflict ? "VOUCH stops" : scenario.id === "human-refund" ? "Human decides" : scenario.failVerification ? "Trust reduces" : "Agent helps"}</small><ArrowRight size={14} /></button>)}</div></section>
    <PageCta title="Simple for people. Sophisticated underneath." text="Strands recommends. VOUCH decides whether the agent may act." label="Explore the architecture" onClick={() => navigate("architecture")} />
  </div>;
}
...
function JudgesPage({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="judges-page content-width">
    <section className="judges-hero"><div><div className="eyebrow"><span className="eyebrow-line" />VOUCH FOR GOOD NEIGHBOR AGENTS</div><h1>An AI teammate.<br /><em>The group keeps the keys.</em></h1><p>A group of people should be able to gain the benefits of an AI teammate without handing that teammate unlimited authority.</p><div className="hero-actions"><button className="button primary" onClick={() => document.getElementById("judges-demo")?.scrollIntoView({ behavior: "smooth" })}><Play size={16} />See VOUCH in action</button><button className="button ghost" onClick={() => navigate("architecture")}>See how it works <ArrowRight size={16} /></button></div></div><div className="judge-stamp"><KnightAuthority level={session.trust.autonomy} size="normal" state="verified" /><span>THE PROOF</span><b>The agent helped.<br /><em>The human stayed in control.</em></b></div></section>
    <section className="judge-brief">{[["WHO IT’S FOR", "Groups of people working together to help other people."], ["THE PROBLEM", "Small teams have more repetitive work than their people have time to handle."], ["THE SOLUTION", "VOUCH lets the group delegate to an AI teammate while keeping important decisions with people."], ["WHY IT’S DIFFERENT", "The teammate earns responsibility through verified performance—and can lose it."], ["HUMAN CONTROL", "The agent cannot authorize itself. Risky work returns to people; unsafe work is blocked."]].map(([title, text]) => <article key={title}><b>{title}</b><p>{text}</p></article>)}</section>
    <section className="judge-takeaways"><div><div className="section-kicker">PROVE → EARN → ACT → VERIFY → ADJUST</div><h2>One teammate.<br /><em>Five accountable outcomes.</em></h2></div><div className="judge-timeline">{[["00:00", "Starts with $250 authority"], ["00:20", "Routine help → verified"], ["00:40", "Responsibility → $500"], ["01:00", "Important action → human"], ["01:20", "Failure → authority reduced"]].map(([time, label], index) => <div key={time} className={index === 4 ? "critical" : ""}><time>{time}</time><span /><b>{label}</b></div>)}</div></section>
    <section className="judge-watch"><div className="watch-panel"><div className="section-kicker">WHAT TO NOTICE</div><h2>Follow who<br /><em>decides what.</em></h2>{["The group delegates limited responsibility.", "Strands investigates and recommends.", "VOUCH independently authorizes.", "Higher-risk work returns to a person.", "Only the protected server mutates Postgres.", "A fresh read verifies the result."].map((item) => <p key={item}><Check size={14} />{item}</p>)}</div><div className="impact-panel"><KnightAuthority level={session.trust.autonomy} size="small" state="verified" /><div className="section-kicker">WHY THIS MATTERS</div><h2>More capacity.<br />Human responsibility.<br /><em>No blind trust.</em></h2><p>VOUCH gives a group more help from AI without giving AI control of the group.</p></div></section>
    ...
    <section className="technical-snapshot"><div><div className="section-kicker">BUILT WITH REAL AGENTIC INFRASTRUCTURE</div><h2>Serious technology.<br /><em>Human control.</em></h2></div><div className="tech-grid">{[...runtime, ["POSTGRESQL", "Durable case · trust · audit state"], ["SERVER-SIDE AUTHORITY", "Fresh deterministic decision per request"], ["HUMAN AUTHORIZATION", "Bound to one exact action"], ["INDEPENDENT VERIFICATION", "Fresh observed state, not agent claims"]].map(([name, text]) => <div key={name}><Layers3 size={16} /><b>{name}</b><span>{text}</span></div>)}</div></section>
  </div>;
}
...
function ControlCenter({ data, runScenario, mutateAction, setToast: _setToast, loading, navigate }: { data: ApiState; runScenario: (id: string) => Promise<void>; mutateAction: (path: string) => Promise<void>; setToast: (s: string) => void; loading: boolean; navigate: (v: View) => void }) {
  ...
  const run = () => runScenario(scenario.id);
   return <div className="control content-width">
      <div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />COMMUNITY-SERVING OPERATIONS / CLAIMS</div><h1>Help more people. Surface the exceptions.</h1><p>The agent handles repetitive claims work while VOUCH keeps decisions that affect people in responsible human hands.</p></div><div className="mode-chip"><i /><span><b>{session.service.mode === "AWS_LIVE" ? "AWS LIVE" : "DETERMINISTIC DEMO FALLBACK"}</b><small>{session.service.message}</small></span></div></div>
...
function Architecture({ navigate }: { navigate: (v: View) => void }) {
  return <div className="architecture content-width">
    <div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />BUILT WITH REAL AGENTIC INFRASTRUCTURE</div><h1>Under the hood:<br /><em>keeping the group in control.</em></h1><p>VOUCH separates what an AI teammate recommends from what it is actually allowed to do.</p></div><div className="architecture-note"><LockKeyhole size={17} /><span><b>The agent cannot authorize itself</b><small>Model output and prior approval never grant future permission.</small></span></div></div>
