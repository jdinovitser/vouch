...
function App() {
  ...
  const runScenario = async (id: string, resolved = false, stayOnDemo = false) => {
    setLoading(true);
    try {
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view === "judges" && data && <JudgesPage scenarios={data.scenarios} runScenario={runScenario} loading={loading} navigate={navigate} />}
...
function JudgesPage({ scenarios, runScenario, loading, navigate }: { scenarios: DemoScenario[]; runScenario: (id: string, resolved?: boolean, stayOnDemo?: boolean) => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  return <div className="judges-page content-width"><section className="judges-hero"><div><div className="eyebrow"><span className="eyebrow-line" />BUILT FOR THE MOMENT OF TRUTH</div><h1>Judge<br /><em>the agent.</em></h1><p>VOUCH is a trust-based autonomy layer for AI agents. Watch it decide whether an agent should act—not simply whether it can.</p><div className="hero-actions"><button className="button primary" onClick={() => document.getElementById("judges-demo")?.scrollIntoView({ behavior: "smooth" })}><Play size={16} />Start 90-second experience</button><button className="button ghost" onClick={() => navigate("landing")}>Explore the product <ArrowRight size={16} /></button></div></div><div className="judge-stamp"><Guardian state="blocked" /><span>WHAT TO WATCH</span><b>The agent actually<br /><em>stops itself.</em></b></div></section><section className="judge-takeaways"><div><div className="section-kicker">THE STORY IN 90 SECONDS</div><h2>Capability is easy to show.<br /><em>Judgment is the demo.</em></h2></div><div className="judge-timeline">{[["00:00", "Agent receives request"], ["00:10", "Evidence collected"], ["00:20", "Risk assessed"], ["00:30", "Conflict detected"], ["00:40", "ACTION BLOCKED"], ["00:50", "Human resolves conflict"], ["01:00", "Action authorized"], ["01:10", "Outcome verified"], ["01:20", "Trust updated"], ["01:30", "Autonomy changes"]].map(([time, label], index) => <div key={time} className={index === 4 ? "critical" : ""}><time>{time}</time><span /><b>{label}</b></div>)}</div></section><section className="judge-watch"><div className="watch-panel"><div className="section-kicker">WHAT TO NOTICE</div><h2>Look for the<br /><em>boundary.</em></h2>{["The agent does not blindly follow instructions.", "Evidence has different authority levels.", "Authorization is enforced outside the model.", "Human approval is used selectively.", "Actions are verified after execution.", "Trust affects future autonomy.", "Failures can reduce authority."].map((item) => <p key={item}><Check size={14} />{item}</p>)}</div><div className="impact-panel"><Guardian state="verified" size="small" /><div className="section-kicker">WHY THIS MATTERS</div><h2>More autonomous work.<br />Less unnecessary review.<br /><em>More useful AI automation.</em></h2><p>The value of agentic AI grows when organizations can safely delegate more work.</p></div></section><section id="judges-demo"><DemoExperience scenarios={scenarios} runScenario={runScenario} loading={loading} navigate={navigate} judgeMode /></section><section className="technical-snapshot"><div><div className="section-kicker">TECHNICAL SNAPSHOT</div><h2>Built for the<br /><em>real world.</em></h2></div><div className="tech-grid">{[["STRANDS AGENTS", "Agent orchestration"], ["AMAZON BEDROCK", "Model inference"], ["AGENT TOOLS", "Evidence · risk · authority · verification"], ["VOUCH POLICY ENGINE", "Deterministic authorization"], ["TRUST ENGINE", "Graduated autonomy"], ["AUDIT LAYER", "Operational provenance"]].map(([name, text]) => <div key={name}><Layers3 size={16} /><b>{name}</b><span>{text}</span></div>)}</div></section></div>;
}

function ControlCenter({ data, runScenario, mutateAction, setToast, loading, navigate }: { data: ApiState; runScenario: (id: string, resolved?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; setToast: (s: string) => void; loading: boolean; navigate: (v: View) => void }) {
...
function Architecture({ navigate }: { navigate: (v: View) => void }) { const nodes = [["USER REQUEST", "Intent enters the trust boundary."], ["STRANDS AGENT", "Recommends and orchestrates tools."], ["TOOLS", "Evidence, risk, policy, action."], ["EVIDENCE LAYER", "Authority and conflict detection."], ["RISK ENGINE", "Impact and reversibility."], ["AUTHORITY ENGINE", "Deterministic allow / ask / block."], ["ACTION", "Only after authorization."], ["VERIFICATION", "Expected state vs actual state."], ["TRUST ENGINE", "Slow promotion. Immediate demotion."], ["AUDIT / OBSERVABILITY", "Append-only operational record."]]; return <div className="architecture content-width"><div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />THE PRODUCT BOUNDARY</div><h1>The agent recommends.<br /><em>VOUCH decides.</em></h1><p>One clear boundary separates model capability from operational authority.</p></div><div className="architecture-note"><LockKeyhole size={17} /><span><b>Policy-controlled</b><small>Model output never grants its own permission.</small></span></div></div><div className="arch-diagram">{nodes.map(([title, text], index) => <div className={`arch-node ${title === "AUTHORITY ENGINE" ? "key-node" : ""}`} key={title}><span className="arch-index">{String(index + 1).padStart(2, "0")}</span><div><b>{title}</b><p>{text}</p></div>{index < nodes.length - 1 && <span className="arch-arrow"><ArrowRight size={15} /></span>}</div>)}</div><div className="arch-bottom"><div><span className="section-kicker">THE BOUNDARY</span><h2>LLM proposes.<br />Policy disposes.</h2></div><p>VOUCH treats evidence as structured data, ranks its authority, calculates action risk, and uses the current trust score to determine whether the system may act autonomously. After action, verification updates the trust state.</p></div><PageCta title="Ready to see it work?" text="Run a controlled scenario with no setup or API key." label="Experience VOUCH" onClick={() => navigate("demo")} /></div>; }
