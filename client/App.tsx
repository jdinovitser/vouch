...
import { authorityKnightLabels, CelebrationKnight, KnightArtwork, KnightAuthority, knightLabels, type KnightState } from "./Knight";
...
function Landing({ navigate, trust }: { navigate: (v: View) => void; trust?: AgentTrust }) {
  return <div className="landing">
    ...
    <section className="why content-width"><div className="section-kicker">HELP WITHOUT THE GUESSWORK</div><h2>AI should help people.<br /><em>Not leave them guessing.</em></h2><p className="section-intro">When an agent acts on someone’s behalf, VOUCH gives people a clear way to understand what it intends to do, whether it should do it, and what happened afterward.</p><div className="principles"><Principle number="01" title="TRUST" text="Is this really what people intended?" icon={<CircleHelp />} /><Principle number="02" title="AUTHORITY" text="Is the agent allowed to do it?" icon={<LockKeyhole />} /><Principle number="03" title="RISK" text="What could happen if it gets it wrong?" icon={<ShieldAlert />} /><Principle number="04" title="VERIFY" text="Can we check what it is relying on?" icon={<FileCheck2 />} /></div></section>
    <CommunityCapacity navigate={navigate} kicker="AI WITH A HUMAN PURPOSE" />
    <TeammateRoles />
    <HumanControl navigate={navigate} />
    <section className="trust-manifesto content-width"><div><span className="section-kicker">RESPONSIBILITY IS EARNED</span><h2>Proof before trust.<br /><span>People where it matters.</span></h2><p>AI earns responsibility through demonstrated, verified behavior—helping people do more while people remain responsible for the decisions that matter.</p></div><button className="text-link" onClick={() => navigate("demo")}>See trust being earned <ArrowRight size={17} /></button></section>
  </div>;
}

function HeroVisual({ level }: { level: AutonomyLevel }) {
  return <div className="hero-visual"><div className="hero-orbit orbit-a" /><div className="hero-orbit orbit-b" /><div className="hero-grid" /><div className="guardian-card authority-card"><div className="guardian-halo hero-warrior"><KnightAuthority level="T3" size="normal" /></div><span className="guardian-label">EARNED RESPONSIBILITY</span><strong className="guardian-level">T3 · ACT</strong><b>Start small. Prove it. Help more.</b><div className="guardian-scan"><i />verified results</div><small className="guardian-proof">The agent earned this responsibility through verified outcomes.</small></div></div>;
}
...
function CommunityCapacity({ navigate, kicker = "A GOOD NEIGHBOR AGENT" }: { navigate: (v: View) => void; kicker?: string }) {
  const groups = [
  ...
  return <section className="community-capacity content-width"><div className="community-heading"><div className="section-kicker">{kicker}</div><h2>Give community teams<br /><em>another set of hands.</em></h2><p>A trusted AI teammate can take on routine work while people stay focused on the relationships, judgment, and decisions only people should make.</p><small>Illustrative use cases—not current customer or deployment claims.</small></div><div className="community-grid">{groups.map(([title, headline, text], index) => <article key={title}><span>0{index + 1}</span><b>{title}</b><h3>{headline}</h3><p>{text}</p></article>)}</div></section>;
}

function TeammateRoles() {
  return <section className="teammate-roles content-width"><div className="section-kicker">THE THREE ROLES</div><h2>Three roles. One shared goal:<br /><em>help people.</em></h2><p className="roles-intro">Each has a clear part in deciding what the AI can responsibly take on.</p><div className="roles-grid"><article><Users size={24} /><span>THE GROUP</span><h3>Decides responsibility.</h3><p>The group sets the boundaries and decides what responsibility the AI should have.</p></article><article><Bot size={24} /><span>THE AI TEAMMATE</span><h3>Proves what it can do.</h3><p>The AI teammate handles real work and builds a track record through results.</p></article><article className="vouch-role"><ShieldCheck size={24} /><span>VOUCH</span><h3>Turns evidence into earned trust.</h3><p>VOUCH connects evidence, risk, and verified outcomes to responsible delegation.</p></article></div><strong>The agent earns responsibility through what it proves.</strong></section>;
}
...
function WhyVouch({ navigate }: { navigate: (v: View) => void }) {
  return <div className="story-page why-page content-width">
    <StoryHero kicker="WHY VOUCH" title={<>People need help.<br /><em>Teams need more hands.</em></>} text="AI can help food pantries, neighborhood groups, schools, and libraries do more with limited time—but it should earn trust before it earns responsibility." mascot="blocked" heroArtwork="warrior" cta="See trust being earned" onClick={() => navigate("demo")} />
    <CommunityCapacity navigate={navigate} kicker="AI WITH A HUMAN PURPOSE" />
    ...
    <section className="value-section teammate-journey" id="teammate-journey"><div><div className="section-kicker">THE TEAMMATE&apos;S JOURNEY</div><h2>Start small.<br /><em>Help more over time.</em></h2><p>PROVE → VERIFY → EARN → ACT → ADJUST</p><small>Verified success can move responsibility forward. Failure can move it back.</small></div><AuthorityJourney /></section>
...
function AuthorityJourney() {
  const levels: Array<[AutonomyLevel, string, string]> = [
    ["T1", "START SMALL", "Observe and learn from limited work."],
    ["T2", "PROVE IT", "Recommend and handle bounded, low-risk actions."],
    ["T3", "EARN ACTION", "Act within authority earned through verified results."],
    ["T4", "HELP MORE", "Take on broader approved work while boundaries remain."],
  ];
  return <div className="journey-progression">{levels.map(([level, title, text], index) => <div className={`journey-level level-${level}`} key={level}><div className="journey-knight"><KnightAuthority level={level} size="small" state="deployment" /></div><span>{level} · {authorityKnightLabels[level].label}</span><b>{title}</b><p>{text}</p>{index < levels.length - 1 && <ArrowRight size={16} />}</div>)}</div>;
}
function RoiSection() {
...
function HowItWorks({ navigate }: { navigate: (v: View) => void }) {
  ...
  return <div className="story-page how-page content-width">
    <StoryHero kicker="HOW VOUCH HELPS" title={<>Let AI handle the busywork.<br /><em>Let people decide what matters.</em></>} text="A community team gets more hands without giving up responsibility. Before its AI teammate acts, VOUCH checks the evidence, boundaries, risk, and whether human judgment is needed." mascot="warrior" cta="See VOUCH in action" onClick={() => navigate("demo")} />
    <section className="lifecycle" id="vouch-backup"><div className="lifecycle-intro"><div className="section-kicker">THE VOUCH BACKUP</div><h2>Every request.<br /><em>A fresh decision.</em></h2><p>The agent helps with the work. VOUCH evaluates the current request. If the answer is not clear, it brings a person back into the decision.</p></div><div className="lifecycle-list">{stages.map(([number, title, text], index) => <div className={`lifecycle-row ${title === "AUTHORITY" ? "authority-row" : ""}`} key={title}><span>{number}</span><div className="lifecycle-marker">{index < 4 ? <Check size={14} /> : <span />}</div><div><b>{title}</b><p>{text}</p></div>{index < stages.length - 1 && <ArrowDown />}</div>)}</div></section>
...
function DemoExperience({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="demo-experience content-width">
    <div className="demo-hero-heading"><div><div className="eyebrow"><span className="eyebrow-line" />VOUCH AI TEAMMATE · A WORKING GOOD NEIGHBOR DEMONSTRATION</div><h1>Helping people,<br /><em>safely.</em></h1><p>A community team gets more hands. With VOUCH, its AI teammate takes on routine work—and earns more responsibility only by proving it can be trusted.</p></div><KnightAuthority level={session.trust.autonomy} size="normal" state="deployment" /></div>
...
function Architecture({ navigate }: { navigate: (v: View) => void }) {
  return <div className="architecture content-width">
    <div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />REAL AGENTIC INFRASTRUCTURE</div><h1>Under the hood:<br /><em>turning evidence into earned trust.</em></h1><p>VOUCH separates what an AI teammate recommends from what the system has evidence to trust it with.</p></div><div className="architecture-note"><LockKeyhole size={17} /><span><b>The agent cannot decide its own trust</b><small>Model output and prior approval never grant future permission.</small></span></div></div>
    <div className="contest-architecture">
      <div className="contest-arch-title"><h2>The agent recommends. <em>VOUCH authorizes.</em></h2><p>Model intelligence is useful input—not permission to act.</p></div>
      <div className="contest-arch-primary">
        <article className="contest-arch-node request-node"><span>01 · REQUEST</span><Users size={22} /><h3>People &amp;<br />community teams</h3><p>A real need enters the system.</p></article>
        <div className="contest-arch-connector"><ArrowRight /><small>REQUEST</small></div>
        <article className="contest-arch-node agent-node"><span>02 · AGENT REASONING</span><Layers3 size={22} /><h3>Strands Agents SDK</h3><p>Amazon Bedrock model path</p><small>Typed, read-only investigation tools</small></article>
        <div className="contest-arch-connector recommendation"><ArrowRight /><small>RECOMMENDATION</small></div>
        <article className="contest-arch-node gate-node"><span>03 · DETERMINISTIC CONTROL</span><ShieldCheck size={25} /><h3>VOUCH Gate</h3><p>Evidence · risk · policy · context</p><p>Earned authority · hard limits</p><strong>FRESH SERVER-SIDE DECISION</strong></article>
        <div className="contest-arch-connector"><ArrowRight /><small>DECISION</small></div>
        <div className="contest-arch-outcomes">
          <div className="execute"><i /><span><b>EXECUTE</b><small>Bounded, reversible work may proceed</small></span></div>
          <div className="approval"><i /><span><b>APPROVAL REQUIRED</b><small>A person decides this exact action</small></span></div>
          <div className="blocked"><i /><span><b>BLOCKED</b><small>Unsafe or untrusted input cannot act</small></span></div>
        </div>
      </div>
      <div className="contest-arch-secondary">
        <article className="contest-arch-node audit-node"><span>06 · DURABLE ACCOUNTABILITY</span><History size={22} /><h3>Audit history</h3><p>Evidence, decisions, approvals, actions, proof</p></article>
        <div className="contest-arch-connector reverse"><ArrowRight /></div>
        <article className="contest-arch-node trust-node"><span>05 · EARNED AUTHORITY</span><BadgeCheck size={22} /><h3>Trust changes capability</h3><p>Verified success earns more · failure revokes it</p></article>
        <div className="contest-arch-connector reverse"><ArrowRight /></div>
        <article className="contest-arch-node verify-node"><span>04 · ACTION + INDEPENDENT VERIFICATION</span><Activity size={22} /><h3>Protected server + Postgres</h3><p>Execute, reload fresh state, compare expected ↔ observed</p></article>
      </div>
      <div className="contest-arch-loop"><RefreshCw size={14} /><span>NEXT ACTION · RE-EVALUATE</span></div>
      <div className="contest-arch-legend"><span className="agent"><i />AI LAYER <small>Strands + Bedrock recommend only</small></span><span className="authority"><i />AUTHORITY LAYER <small>Deterministic and server-bound</small></span><span className="proof"><i />PROOF LOOP <small>Postgres verification + audit</small></span></div>
      <p className="contest-arch-disclosure">Current prototype architecture. Amazon Bedrock AgentCore is not claimed as deployed.</p>
    </div>
