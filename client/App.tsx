...
function CommunityCapacity({ navigate, kicker = "A GOOD NEIGHBOR AGENT" }: { navigate: (v: View) => void; kicker?: string }) {
  ...
  return <section className="community-capacity content-width"><div className="community-heading"><div className="section-kicker">{kicker}</div><h2>Give community teams<br /><em>another set of hands.</em></h2><p>A small team should not have to choose between helping more people and protecting the time of the people doing the work. A trusted AI teammate can take on routine work while people stay focused on the relationships, judgment, and decisions only people should make.</p><button className="text-link" onClick={() => navigate("how")}>See how trust is earned <ArrowRight size={17} /></button><small>Illustrative use cases—not current customer or deployment claims.</small></div><div className="community-grid">{groups.map(([title, headline, text], index) => <article key={title}><span>0{index + 1}</span><b>{title}</b><h3>{headline}</h3><p>{text}</p></article>)}</div></section>;
}
