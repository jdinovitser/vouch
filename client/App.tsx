...
function HeroVisual({ level }: { level: AutonomyLevel }) {
  ...
  return <div className="hero-visual"><div className="hero-orbit orbit-a" /><div className="hero-orbit orbit-b" /><div className="hero-grid" /><div className="guardian-card authority-card"><div className="guardian-halo hero-warrior"><KnightAuthority level="T3" size="normal" /></div><span className="guardian-label">EARNED RESPONSIBILITY / T3</span><b>Start small. Prove it. Help more.</b><div className="guardian-scan"><i />verified results <span>T3 · ACT</span></div></div><div className="trust-float"><div className="mini-label">CURRENT EARNED LEVEL</div><div className="float-score">{level}<span> · {autonomyName(level)}</span></div><div className="float-track"><i style={{ width: `${level === "T1" ? 25 : level === "T2" ? 50 : level === "T3" ? 75 : 100}%` }} /></div><div className="float-row"><span>{statusCopy}</span><b>VERIFIED</b></div></div></div>;
}
