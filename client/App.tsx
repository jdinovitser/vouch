...
function App() {
  ...
  const resetSession = async () => {
    ...
    try {
      ...
      setToast("Earned autonomy demonstration reset.");
...
function EarnedAutonomyStrip({ view }: { view: View }) {
  const copy = view === "judges"
    ...
    : view === "demo"
      ? "The signature proof: EARN → ACT → VERIFY → LOSE → RECOVER."
      : view === "control"
        ? "What may this agent do now? Its current authority is the consequence of its verified history."
        : view === "history"
          ? "Read every transition as outcome → trust → authority. This is the agent's operational memory."
          : view === "architecture"
            ? "The feedback loop is enforced outside the model: verified outcomes update trust, then future authority."
      : "Authority is not assigned once. It is earned from verified behavior, then applied to the next decision.";
  return <div className="earned-banner"><span className="section-kicker">VOUCH / EARNED AUTONOMY</span><h2>{copy} <em>Past behavior changes future permission.</em></h2></div>;
}
...
function Footer({ navigate }: { navigate: (v: View) => void }) {
  return <footer><div className="footer-brand"><img src="/vouch-mark.svg" /> VOUCH</div><div className="footer-links"><button onClick={() => navigate("why")}>Why VOUCH</button><button onClick={() => navigate("how")}>How it works</button><button onClick={() => navigate("demo")}>Demo</button><button onClick={() => navigate("judges")}>For judges</button></div><span>Earned autonomy for AI agents</span></footer>;
}

function Landing({ navigate }: { navigate: (v: View) => void }) {
  return <div className="landing">
    <section className="landing-hero content-width">
      <div className="hero-copy"><div className="eyebrow"><span className="eyebrow-line" />EARNED AUTONOMY INFRASTRUCTURE</div><h1>AI agents don't need<br /><em>unlimited autonomy.</em><br />They need a way to earn it.</h1><p className="hero-lead">VOUCH gives agents verifiable authority that changes with what they prove they can do. The past outcome shapes the next permission.</p><div className="earned-loop"><b>PROVE</b><i /><b>EARN</b><i /><b>ACT</b><i /><b>VERIFY</b><i /><b>ADJUST</b></div><div className="hero-actions"><button className="button primary" onClick={() => navigate("control")}><Zap size={16} />See what this agent may do <ArrowRight size={16} /></button><button className="button ghost" onClick={() => navigate("demo")}>Watch authority change <ChevronRight size={16} /></button></div><div className="hero-proof"><span><ShieldCheck size={16} /> Authority is earned</span><span><FileCheck2 size={16} /> Outcomes are verified</span><span><Fingerprint size={16} /> State is server-held</span></div></div>
      <HeroVisual />
    </section>
    ...
    <section className="trust-manifesto content-width"><div><span className="section-kicker">THE VOUCH PROMISE</span><h2>The goal isn’t maximum autonomy.<br /><span>It’s earned authority.</span></h2></div><button className="text-link" onClick={() => navigate("demo")}>Watch the same agent change state <ArrowRight size={17} /></button></section>
  </div>;
}

function HeroVisual() {
  return <div className="hero-visual"><div className="hero-orbit orbit-a" /><div className="hero-orbit orbit-b" /><div className="hero-grid" /><div className="guardian-card authority-card"><div className="guardian-halo"><Guardian state="default" /></div><span className="guardian-label">CURRENT AUTHORITY CREDENTIAL</span><b>Earned, exercised,<br />continuously reviewed.</b><div className="guardian-scan"><i />agent standing <span>VERIFIED</span></div></div><div className="trust-float"><div className="mini-label">CURRENT TRUST</div><div className="float-score">84<span>/100</span></div><div className="float-track"><i /></div><div className="float-row"><span>T2 · RECOMMEND</span><b>READY TO EARN</b></div></div></div>;
}
...
function WhyVouch({ navigate }: { navigate: (v: View) => void }) {
  return <div className="story-page why-page content-width"><StoryHero kicker="WHY VOUCH" title={<>Permissions say what an agent can do.<br /><em>VOUCH proves what it has earned.</em></>} text="Static credentials cannot account for whether the last action actually worked. VOUCH makes verified behavior part of the next authorization decision." mascot="blocked" cta="Watch authority change" onClick={() => navigate("demo")} /><section className="story-section"><div className="section-kicker">THE AUTONOMY DILEMMA</div><h2>Too little autonomy is slow.<br />Too much is risky.</h2><p className="story-lead">An agent that can modify records, approve transactions, trigger workflows, or change system state has become an operational actor—not traditional assistance.</p><div className="option-grid"><Option title="NO AUTONOMY" tag="Safe, but slow." text="Every action requires a human. Humans become the bottleneck and automation becomes glorified assistance." tone="muted" /><Option title="STATIC PERMISSION" tag="Fast, but blind." text="The credential stays the same even after a bad outcome. Capability is mistaken for earned authority." tone="danger" /><Option title="EARNED AUTONOMY" tag="Adaptive and controlled." text="Verified behavior changes standing. Standing changes the next permission." tone="success" /></div></section><section className="value-section"><div><div className="section-kicker">WHAT VOUCH CHANGES</div><h2>Trust is not a one-time<br /><em>permission decision.</em></h2></div><div className="change-list"><Change from="Every consequential action needs a human." to="Humans focus on actions that actually require judgment." /><Change from="Agents receive static permissions." to="Authority adapts to demonstrated reliability." /><Change from="Success means the API call returned." to="Success means the expected outcome was verified." /><Change from="Trust is assumed." to="Trust is earned, lost, and recovered." /></div></section><RoiSection /><section className="why-now"><div className="section-kicker">WHY NOW</div><h2>When AI can act,<br /><em>authorization becomes part of the product.</em></h2><div className="automation-ladder"><span>Traditional automation <b>executes a predefined workflow</b></span><ArrowRight /><span>Generative AI <b>produces an answer</b></span><ArrowRight /><span className="current">Agentic AI <b>earns changing authority</b></span></div></section><PageCta title="Let reliable agents handle more work." text="VOUCH reserves human judgment for the decisions that actually need it." label="See how it works" onClick={() => navigate("how")} /></div>;
}
...
function DemoExperience({ session, scenarios, runScenario, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="demo-experience content-width">
    <div className="demo-hero-heading"><div><div className="eyebrow"><span className="eyebrow-line" />THE EARNED AUTONOMY PROOF</div><h1>One agent.<br /><em>Changing authority.</em></h1><p>Run the real deterministic lifecycle: earn the right to act, lose it after a failed outcome, then recover it through verified behavior.</p></div><Guardian state={session.trust.autonomy === "T3" ? "verified" : "default"} /></div>
...
function JudgesPage({ session, scenarios, runScenario, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="judges-page content-width"><section className="judges-hero"><div><div className="eyebrow"><span className="eyebrow-line" />THE 90-SECOND PROOF</div><h1>Watch authority<br /><em>become consequential.</em></h1><p>The same Claims Resolution Agent gains the right to act, loses it when verification fails, and earns it back through a monitored recovery sequence.</p><div className="hero-actions"><button className="button primary" onClick={() => document.getElementById("judges-demo")?.scrollIntoView({ behavior: "smooth" })}><Play size={16} />Start live proof</button><button className="button ghost" onClick={() => navigate("architecture")}>Inspect the boundary <ArrowRight size={16} /></button></div></div><div className="judge-stamp"><Guardian state="verified" /><span>WHAT TO WATCH</span><b>Outcome changes trust.<br /><em>Trust changes permission.</em></b></div></section><section className="judge-takeaways"><div><div className="section-kicker">THE STORY IN 90 SECONDS</div><h2>Not just a decision.<br /><em>A changing relationship.</em></h2></div><div className="judge-timeline">{[["00:00", "T2 · Recommend"], ["00:15", "Earn T3 · Act"], ["00:30", "Action executes"], ["00:45", "Verification fails"], ["01:00", "Demoted to T2"], ["01:15", "Approval now required"], ["01:30", "Recover T3"]].map(([time, label], index) => <div key={time} className={index === 4 ? "critical" : ""}><time>{time}</time><span /><b>{label}</b></div>)}</div></section><section className="judge-watch"><div className="watch-panel"><div className="section-kicker">WHAT TO NOTICE</div><h2>Look for the<br /><em>consequence.</em></h2>{["The agent begins at T2 RECOMMEND.", "A verified outcome earns T3 ACT.", "The model never grants its own authority.", "A failed outcome immediately demotes the agent.", "The same action class now needs human approval.", "Verified recovery restores authority."].map((item) => <p key={item}><Check size={14} />{item}</p>)}</div><div className="impact-panel"><Guardian state="verified" size="small" /><div className="section-kicker">WHY THIS MATTERS</div><h2>Useful autonomy.<br />Visible accountability.<br /><em>A path back to trust.</em></h2><p>Organizations can delegate more work without making authority permanent or recovery arbitrary.</p></div></section><section id="judges-demo"><EarnedLifecycleDemo session={session} scenarios={scenarios} runScenario={runScenario} resetSession={resetSession} loading={loading} /></section><section className="technical-snapshot"><div><div className="section-kicker">TECHNICAL SNAPSHOT</div><h2>Built for the<br /><em>real world.</em></h2></div><div className="tech-grid">{[...runtime, ["AGENT TOOLS", "Evidence inspection · recommendation"], ["VOUCH POLICY ENGINE", "Deterministic authorization"], ["TRUST ENGINE", "Outcome-driven autonomy"], ["AUDIT LAYER", "Operational provenance"]].map(([name, text]) => <div key={name}><Layers3 size={16} /><b>{name}</b><span>{text}</span></div>)}</div></section></div>;
}
...
function EarnedLifecycleDemo({ session, scenarios, runScenario, resetSession, loading }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; resetSession: () => Promise<void>; loading: boolean }) {
  ...
  const knight: KnightState = phase === 2 ? "reduced" : phase === 3 ? "approval" : phase >= 4 ? "verified" : phase === 0 ? "default" : "deployment";
...
function ControlCenter({ data, runScenario, mutateAction, setToast, loading, navigate }: { data: ApiState; runScenario: (id: string) => Promise<void>; mutateAction: (path: string) => Promise<void>; setToast: (s: string) => void; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="control content-width"><div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />CLAIMS RESOLUTION AGENT / LIVE SESSION</div><h1>What may this agent do now?</h1><p>Its current authority is the direct consequence of verified behavior.</p></div><div className="mode-chip"><i /><span><b>{session.service.mode === "AWS_LIVE" ? "AWS LIVE" : "DEMO MODE"}</b><small>{session.service.message}</small></span></div></div><AuthorityCredential trust={session.trust} /><section className="state-strip"><TrustPanel trust={session.trust} /><div className="state-divider" /><div className="state-meta"><span className="mini-label">CURRENT AUTHORITY</span><b>{session.trust.autonomy} · {autonomyName(session.trust.autonomy)}</b><p>{session.trust.autonomy === "T3" || session.trust.autonomy === "T4" ? "Verified medium-risk actions may execute within policy." : session.trust.autonomy === "T2" ? "Low-risk reversible actions may execute; consequential actions require review." : "The agent may observe and recommend, but not change external state."}</p><div className="level-dots"><i className={session.trust.autonomy === "T1" ? "on" : ""}>T1</i><i className={session.trust.autonomy === "T2" ? "on" : ""}>T2</i><i className={session.trust.autonomy === "T3" ? "on" : ""}>T3</i><i className={session.trust.autonomy === "T4" ? "on" : ""}>T4</i></div></div><div className="state-meta state-meta-right"><span className="mini-label">EARNED STANDING</span><div className="health-value"><i className="pulse" />Continuously evaluated</div><p>Every outcome can change the next authority decision.</p><div className="health-counters"><span><b>{session.audit.length}</b> audit events</span><span><b>{session.trust.verifiedActions}</b> verified actions</span></div></div></section><Workflow state={current?.state} /><div className="dashboard-grid"><DecisionCard current={current} scenario={scenario} blocked={blocked} approval={approval} verified={verified} failed={failed} run={run} resolve={() => current && mutateAction(`/api/actions/${current.action.id}/resolve`)} mutateAction={mutateAction} loading={loading} /><ActivityPanel session={session} /></div><div className="lower-grid"><EvidencePanel evidence={session.evidence.length ? session.evidence : scenario.evidence} /><TrustHistory trust={session.trust} events={session.trustHistory} /></div><button className="back-to-demo" onClick={() => navigate("demo")}><Play size={14} /> Return to earned autonomy demo</button></div>;
}

function AuthorityCredential({ trust }: { trust: AgentTrust }) {
  const expanded = trust.autonomy === "T3" || trust.autonomy === "T4";
  return <section className="authority-credential-live"><div><span className="mini-label">LIVE AUTHORITY CREDENTIAL</span><h2>{trust.autonomy} · {autonomyName(trust.autonomy)}</h2><p>Claims Resolution Agent · Trust {trust.score}/100</p></div><div className="credential-scope"><span><b>ALLOWED NOW</b>{expanded ? "Low-risk and eligible medium-risk actions" : trust.autonomy === "T2" ? "Investigation and low-risk reversible actions" : "Observation and recommendation"}</span><span><b>REQUIRES REVIEW</b>{expanded ? "High-risk or irreversible actions" : "Medium- and high-risk actions"}</span><span><b>ALWAYS BLOCKED</b>Conflicted evidence, unsafe instructions, insufficient support</span></div></section>;
}
...
function Architecture({ navigate }: { navigate: (v: View) => void }) {
  return <div className="architecture content-width">
    ...
    <section className="feedback-loop-box"><div className="section-kicker">THE EARNED AUTONOMY LOOP</div><b>OUTCOME → TRUST → AUTHORITY → NEXT ACTION</b><p>Verification is not the final reporting step. It writes the standing that the authority engine reads on the next request.</p></section>
    <section className="arch-contest-proof"><div className="arch-section-heading"><span className="section-kicker">CONTEST PROOF</span><h2>What this build<br /><em>demonstrates.</em></h2></div><div className="arch-proof-grid"><div><span>01</span><b>AGENTIC AI</b><p>Strands coordinates evidence inspection and produces a structured recommendation.</p></div><div><span>02</span><b>RESPONSIBLE ACTION</b><p>Server-side policy prevents model output from granting permission.</p></div><div><span>03</span><b>EARNED AUTONOMY</b><p>Verified outcomes change trust and future authority in both directions.</p></div><div><span>04</span><b>JUDGE-READY PROOF</b><p>A deterministic lifecycle shows authority earned, lost, and recovered without fake AWS claims.</p></div></div></section>
...
function HistoryView({ session, navigate }: { session: SessionState; navigate: (v: View) => void }) { return <div className="history-page content-width"><div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />AUTHORITY HISTORY / AUDIT</div><h1>How this agent earned<br /><em>its current authority.</em></h1><p>Every verified outcome becomes an explainable trust transition and a future permission consequence.</p></div><div className="history-guardian"><Guardian state="audit" size="small" /><div className="history-summary"><b>{session.trust.autonomy}</b><span>{autonomyName(session.trust.autonomy)} · trust {session.trust.score}</span></div></div></div><section className="authority-history"><div className="panel-heading"><div><span className="section-label">OUTCOME → TRUST → AUTHORITY</span><p>Latest transition first</p></div><span className="source-count">{session.trustHistory.length} CHANGES</span></div>{session.trustHistory.length ? <div className="authority-transition-list">{session.trustHistory.map((event) => <div className={`authority-transition ${event.to < event.from ? "lost" : "earned"}`} key={event.id}><span>{event.to < event.from ? "AUTHORITY REDUCED" : event.autonomyFrom !== event.autonomyTo ? "AUTHORITY CHANGED" : "STANDING UPDATED"}</span><b>{event.from} → {event.to}</b><ArrowRight size={14} /><strong>{event.autonomyFrom} {autonomyName(event.autonomyFrom)} → {event.autonomyTo} {autonomyName(event.autonomyTo)}</strong><p>{event.reason}</p></div>)}</div> : <div className="empty-history"><History size={22} /><b>No authority transitions yet</b><span>Begin the earned autonomy proof to create the first outcome-driven change.</span></div>}</section><div className="audit-table"><div className="audit-head"><span>EVENT</span><span>ACTION</span><span>STATUS</span><span>RISK</span><span>TIME</span></div>{session.audit.length ? session.audit.map((event) => <div className="audit-row" key={event.id}><div><span className="audit-icon"><Check size={14} /></span><b>{event.type.replaceAll("_", " ")}</b><small>{event.actor} · {event.result}</small></div><span className="mono">{event.actionId.replace("act-", "")}</span><span className={`audit-status ${event.status.toLowerCase()}`}>{event.status}</span><span className={`risk-badge ${event.risk.toLowerCase()}`}>{event.risk}</span><time>{new Date(event.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time></div>) : <div className="empty-history"><History size={22} /><b>No audit events yet</b><span>Run a scenario to create the first operational record.</span></div>}</div><button className="back-to-demo" onClick={() => navigate("demo")}><Play size={14} /> Experience the earned autonomy proof</button></div>; }
