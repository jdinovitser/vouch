...
function DemoExperience({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="demo-experience content-width">
    <div className="demo-hero-heading"><div><div className="eyebrow"><span className="eyebrow-line" />WATCH TRUST BEING EARNED</div><h1>The agent proves itself.<br /><em>Responsibility adjusts.</em></h1><p>Our claims-resolution scenario shows how an AI teammate earns responsibility through successful, verified work—and loses responsibility when the evidence says it should.</p></div><KnightAuthority level={session.trust.autonomy} size="normal" state="deployment" /></div>
