...
function CommunityCapacity({ navigate }: { navigate: (v: View) => void }) {
  const groups = [
    ["NEIGHBORHOOD TEAMS", "Support more neighbors.", "More capacity to coordinate requests and support neighbors."],
    ["FOOD PANTRY TEAMS", "Give time back to volunteers.", "Less repetitive work, more time serving families."],
    ["SCHOOL + LIBRARY TEAMS", "Make routine coordination lighter.", "More help with administrative work while people focus on relationships."],
    ["SMALL LOCAL ORGANIZATIONS", "Add a trusted set of hands.", "More help without asking people to blindly trust an AI."],
  ];
  return <section className="community-capacity content-width"><div className="community-heading"><div className="section-kicker">A GOOD NEIGHBOR AGENT</div><h2>More hands for<br /><em>the people doing the work.</em></h2><p>A small group should not have to choose between serving more people and protecting its time. A trusted AI teammate can take on routine work while people stay focused on the decisions and relationships that matter.</p><button className="text-link" onClick={() => navigate("how")}>See how trust is earned <ArrowRight size={17} /></button><small>Illustrative use cases—not current customer or deployment claims.</small></div><div className="community-grid">{groups.map(([title, headline, text], index) => <article key={title}><span>0{index + 1}</span><b>{title}</b><h3>{headline}</h3><p>{text}</p></article>)}</div></section>;
}
...
function WhyVouch({ navigate }: { navigate: (v: View) => void }) {
  return <div className="story-page why-page content-width">
    <StoryHero kicker="WHY VOUCH" title={<>How can a group trust<br /><em>an AI teammate?</em></>} text="Not by assuming the agent is trustworthy. By giving it opportunities to prove itself through real work and independently verified outcomes." mascot="blocked" heroArtwork="warrior" cta="See trust being earned" onClick={() => navigate("demo")} />
    <section className="story-section"><div className="section-kicker">TRUST THROUGH EVIDENCE</div><h2>Your AI teammate should earn<br /><em>greater responsibility—not simply receive it.</em></h2><p className="story-lead">VOUCH gives groups a way to trust AI through evidence instead of blind delegation.</p><div className="option-grid"><Option title="PROVEN" tag="Work creates evidence." text="The agent builds a real record by handling limited tasks." tone="muted" /><Option title="VERIFIED" tag="Outcomes are checked." text="Important results are verified independently—not accepted from the agent." tone="success" /><Option title="EARNED + ADAPTIVE" tag="Responsibility follows performance." text="Successful work can increase responsibility; failed work can reduce it." tone="success" /><Option title="HUMAN + SAFE" tag="Judgment remains human." text="Higher stakes return to people, and hard boundaries stop unsafe work." tone="danger" /></div></section>
    ...
    <PageCta title="The agent proves itself. The group decides what it has earned." text="Trust grows through successful, independently verified work." label="See trust being earned" onClick={() => navigate("demo")} />
  </div>;
}
...
function DemoExperience({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="demo-experience content-width">
    <div className="demo-hero-heading"><div><div className="eyebrow"><span className="eyebrow-line" />A WORKING GOOD NEIGHBOR DEMONSTRATION</div><h1>Helping people,<br /><em>safely.</em></h1><p>A small community team is helping people with limited time and resources. VOUCH gives its AI teammate room to help—while making sure trust is earned before it takes on more responsibility.</p></div><KnightAuthority level={session.trust.autonomy} size="normal" state="deployment" /></div>
    ...
    <section className="demo-outcome"><div><span className="section-kicker">THE HEART OF THE DEMO</span><h2>The agent&apos;s responsibility changes<br /><em>because its evidence changes.</em></h2><p>See verified work earn more trust, higher stakes return to a person, failed verification reduce responsibility, and unsafe instructions stop.</p></div><div className="scenario-rail">{scenarios.filter((scenario) => !scenario.recovery && scenario.id !== "recovered-account-update").map((scenario) => <button className={`scenario-mini ${scenario.accent} ${scenario.id === scenarioId ? "selected" : ""}`} onClick={() => { setScenarioId(scenario.id); void runScenario(scenario.id); }} disabled={loading} key={scenario.id}><span className="mini-label">{scenario.action.risk} RISK</span><b>{scenario.name}</b><small>{scenario.hasConflict ? "VOUCH stops" : scenario.id === "human-refund" ? "Human decides" : scenario.failVerification ? "Trust reduces" : "Agent helps"}</small><ArrowRight size={14} /></button>)}</div></section>
    <PageCta title="The AI can help. Responsibility is earned, verified, and bounded by people." text="Underneath: Strands recommends, VOUCH evaluates, the server acts, and a fresh read verifies the result." label="Explore the architecture" onClick={() => navigate("architecture")} />
  </div>;
}
...
function JudgesPage({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="judges-page content-width">
    <section className="judges-hero"><div><div className="eyebrow"><span className="eyebrow-line" />VOUCH — GOOD NEIGHBOR AGENTS</div><h1>Trust is earned<br /><em>through proven work.</em></h1><p>VOUCH gives groups of people an AI teammate they can learn to trust through proven, verified work.</p><div className="hero-actions"><button className="button primary" onClick={() => document.getElementById("judges-demo")?.scrollIntoView({ behavior: "smooth" })}><Play size={16} />See trust being earned</button><button className="button ghost" onClick={() => navigate("architecture")}>See how it works <ArrowRight size={16} /></button></div></div><div className="judge-stamp"><KnightAuthority level={session.trust.autonomy} size="normal" state="verified" /><span>THE GOOD NEIGHBOR PROOF</span><b>The agent proves itself.<br /><em>The group decides what it has earned.</em></b></div></section>
    <section className="judge-brief">{[["WHO IT’S FOR", "Groups of people working together to help other people."], ["THE PROBLEM", "They need more capacity, but cannot blindly trust an AI teammate with increasing responsibility."], ["THE SOLUTION", "The teammate starts small, proves its ability, earns responsibility, and returns important decisions to people."], ["WHY IT MATTERS", "Trusted routine help gives people more time to focus on the people they serve."], ["WHAT’S DIFFERENT", "Trust is dynamic: verified success can increase responsibility; failed verification can reduce it."]].map(([title, text]) => <article key={title}><b>{title}</b><p>{text}</p></article>)}</section>
    ...
    <section className="judge-watch"><div className="watch-panel"><div className="section-kicker">WHAT TO NOTICE</div><h2>Follow how trust<br /><em>becomes responsibility.</em></h2>{["The group begins with limited responsibility.", "Strands investigates and recommends.", "VOUCH independently evaluates what the evidence supports.", "Higher-risk work returns to a person.", "A fresh read verifies the real result.", "Success or failure changes what the teammate can handle next."].map((item) => <p key={item}><Check size={14} />{item}</p>)}</div><div className="impact-panel"><KnightAuthority level={session.trust.autonomy} size="small" state="verified" /><div className="section-kicker">WHY THIS MATTERS</div><h2>More capacity.<br />Earned responsibility.<br /><em>No blind trust.</em></h2><p>When routine work can be trusted to an AI teammate, people have more time to focus on the people they serve.</p></div></section>
    ...
    <section className="technical-snapshot"><div><div className="section-kicker">BUILT WITH REAL AGENTIC INFRASTRUCTURE</div><h2>Serious technology.<br /><em>Earned trust.</em></h2></div><div className="tech-grid">{[...runtime, ["POSTGRESQL", "Durable case · trust · audit state"], ["SERVER-SIDE AUTHORITY", "Fresh deterministic decision per request"], ["HUMAN AUTHORIZATION", "Bound to one exact action"], ["INDEPENDENT VERIFICATION", "Fresh observed state, not agent claims"]].map(([name, text]) => <div key={name}><Layers3 size={16} /><b>{name}</b><span>{text}</span></div>)}</div></section>
  </div>;
}
