...
const titleByView: Record<View, string> = {
  landing: "VOUCH — The AI That Looks Out for You", why: "VOUCH — Why People Need VOUCH", how: "VOUCH — How It Helps",
...
function Header({ view, navigate, mobileNav, setMobileNav, trust }: { view: View; navigate: (v: View) => void; mobileNav: boolean; setMobileNav: (v: boolean) => void; trust?: AgentTrust }) {
  ...
  return <header className={`topbar ${publicView ? "public-nav" : "app-nav"}`}>
    <button className="brand" onClick={() => navigate("landing")} aria-label="VOUCH home"><img src="/vouch-mark.svg" /><span>VOUCH<small>Looks out for people</small></span></button>
    ...
    <nav className={mobileNav ? "nav open" : "nav"}>
      ...
      <button className={view === "how" ? "active" : ""} onClick={() => navigate("how")}>How it helps</button>
      <button className={view === "demo" ? "active" : ""} onClick={() => navigate("demo")}><Play size={14} />See it in action</button>
...
function Footer({ navigate }: { navigate: (v: View) => void }) {
  return <footer><div className="footer-brand"><img src="/vouch-mark.svg" /> VOUCH</div><div className="footer-links"><button onClick={() => navigate("why")}>Why VOUCH</button><button onClick={() => navigate("how")}>How it helps</button><button onClick={() => navigate("demo")}>See it in action</button><button onClick={() => navigate("judges")}>For judges</button></div><span>The AI that looks out for you</span></footer>;
}

function Landing({ navigate, trust }: { navigate: (v: View) => void; trust?: AgentTrust }) {
  return <div className="landing">
    <section className="landing-hero content-width">
      <div className="hero-copy"><div className="eyebrow"><span className="eyebrow-line" />AI SUPPORT FOR PEOPLE + COMMUNITIES</div><h1>The AI that<br /><em>looks out for you.</em></h1><p className="hero-lead">AI agents can help people and communities get things done. VOUCH gives them a trusted second set of eyes before an agent takes an important action—checking what it wants to do, what could go wrong, and when a person needs to decide.</p><div className="earned-loop"><b>HELP</b><i /><b>CHECK</b><i /><b>DECIDE</b><i /><b>SERVE</b></div><div className="hero-actions"><button className="button primary" onClick={() => navigate("demo")}><Zap size={16} />See VOUCH in action <ArrowRight size={16} /></button><button className="button ghost" onClick={() => navigate("why")}>Why VOUCH? <ChevronRight size={16} /></button></div><div className="hero-proof"><span><ShieldCheck size={16} /> AI handles routine work</span><span><Users size={16} /> People decide what matters</span><span><Fingerprint size={16} /> Every outcome is verified</span></div></div>
...
function CommunityCapacity({ navigate }: { navigate: (v: View) => void }) {
  const groups = [
    ["NONPROFITS", "Do more with limited staff time.", "Routine coordination can move forward while important decisions stay with the people serving others."],
    ["SCHOOLS", "Give teams more capacity without giving up responsibility.", "AI can help with repetitive work while educators remain accountable for decisions that affect students."],
    ["LIBRARIES", "Make useful AI feel safer to share.", "People can benefit from assistance with a clear human boundary when the answer is not enough."],
    ["LOCAL ORGANIZATIONS", "Serve more people with the team you have.", "Community groups can use powerful agents without needing a massive governance department."],
  ];
  return <section className="community-capacity content-width"><div className="community-heading"><div className="section-kicker">FOR THE PEOPLE DOING THE WORK</div><h2>AI can give communities<br /><em>more capacity.</em></h2><p>Small organizations and community-serving groups often have big needs and limited time. VOUCH helps bridge the gap between useful automation and human control.</p><button className="text-link" onClick={() => navigate("how")}>See how VOUCH helps <ArrowRight size={17} /></button></div><div className="community-grid">{groups.map(([title, headline, text], index) => <article key={title}><span>0{index + 1}</span><b>{title}</b><h3>{headline}</h3><p>{text}</p></article>)}</div></section>;
}

function WhyVouch({ navigate }: { navigate: (v: View) => void }) {
  return <div className="story-page why-page content-width"><StoryHero kicker="WHY PEOPLE NEED VOUCH" title={<>Because AI should help people.<br /><em>Not leave them guessing.</em></>} text="Agents can expand what people and community-serving organizations accomplish. VOUCH is the backup that checks what an agent wants to do, whether it should be allowed, and when a person needs to make the final call." mascot="blocked" heroArtwork="warrior" cta="See VOUCH in action" onClick={() => navigate("demo")} /><section className="story-section"><div className="section-kicker">THE HUMAN QUESTION</div><h2>People deserve a second set of eyes<br /><em>when AI acts for them.</em></h2><p className="story-lead">Helpful automation creates a new question: what happens when an agent is about to do something important, risky, or outside what someone intended? VOUCH helps people benefit from AI without having to face its decisions alone.</p><div className="option-grid"><Option title="NO BACKUP" tag="Safe, but slow." text="Every consequential action reaches a person. The people doing the work become the bottleneck." tone="muted" /><Option title="UNBOUNDED AI" tag="Fast, but brittle." text="The agent keeps acting even after a bad outcome or a change in what people intended." tone="danger" /><Option title="VOUCH AS BACKUP" tag="Helpful and accountable." text="AI handles routine work, while evidence, boundaries, and human judgment protect what matters." tone="success" /></div></section><section className="value-section"><div><div className="section-kicker">THE FOUR QUESTIONS</div><h2>Before AI acts,<br /><em>VOUCH checks.</em></h2></div><div className="change-list"><Change from="Is this what we intended?" to="Trust keeps the action connected to the people it serves." /><Change from="Is the agent allowed to do it?" to="Authority stays earned, bounded, and revocable." /><Change from="What happens if it gets it wrong?" to="Risk determines how much human attention is needed." /><Change from="Can we verify what it relied on?" to="Fresh evidence and outcomes keep the record accountable." /></div></section><CommunityCapacity navigate={navigate} /><RoiSection /><section className="why-now"><div className="section-kicker">WHEN AI CAN ACT</div><h2>Let AI handle the busywork.<br /><em>Let people decide what matters.</em></h2><div className="automation-ladder"><span>People have a need <b>limited time and shared responsibility</b></span><ArrowRight /><span>AI agent helps <b>repetitive work moves faster</b></span><ArrowRight /><span className="current">VOUCH looks out <b>human control stays visible</b></span></div></section><PageCta title="See how VOUCH helps." text="AI can do more for people when it does not decide instead of them." label="See VOUCH in action" onClick={() => navigate("demo")} /></div>;
}
...
function HowItWorks({ navigate }: { navigate: (v: View) => void }) {
  ...
  return <div className="story-page how-page content-width">
    ...
    <section className="trust-product"><CelebrationKnight size="small" /><div><div className="section-kicker">THE FEEDBACK LOOP</div><h2>HELP → CHECK → DECIDE.<br /><em>SERVE → VERIFY → ADJUST.</em></h2></div><div className="trust-loop"><span>Community need</span><ArrowRight /><b>Agent helps</b><ArrowRight /><span>VOUCH checks</span><strong>Human decides</strong><span>Protected action</span><ArrowRight /><b>Fresh verification</b><ArrowRight /><span>More capacity</span></div></section>
...
function DemoExperience({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="demo-experience content-width">
    <div className="demo-hero-heading"><div><div className="eyebrow"><span className="eyebrow-line" />SEE VOUCH IN ACTION</div><h1>The agent helps.<br /><em>People stay in control.</em></h1><p>Follow a community-serving team as its AI handles routine work, pauses for human judgment, and proves what happened.</p></div><KnightAuthority level={session.trust.autonomy} size="normal" state="deployment" /></div>
...
function JudgesPage({ session, scenarios, runScenario, mutateAction, resetSession, loading, navigate }: { session: SessionState; scenarios: DemoScenario[]; runScenario: (id: string, stayOnDemo?: boolean) => Promise<void>; mutateAction: (path: string) => Promise<void>; resetSession: () => Promise<void>; loading: boolean; navigate: (v: View) => void }) {
  ...
  return <div className="judges-page content-width"><section className="judges-hero"><div><div className="eyebrow"><span className="eyebrow-line" />THE 90-SECOND COMMUNITY PROOF</div><h1>AI handles the routine.<br /><em>People decide what matters.</em></h1><p>A community-serving team uses an AI agent to move repetitive claims work forward. VOUCH looks out for the people affected by checking boundaries, returning important decisions to humans, and proving what actually happened.</p><div className="hero-actions"><button className="button primary" onClick={() => document.getElementById("judges-demo")?.scrollIntoView({ behavior: "smooth" })}><Play size={16} />See VOUCH in action</button><button className="button ghost" onClick={() => navigate("architecture")}>See how it works <ArrowRight size={16} /></button></div></div><div className="judge-stamp"><KnightAuthority level={session.trust.autonomy} size="normal" state="verified" /><span>WHAT TO WATCH</span><b>The agent helped.<br /><em>The human stayed in control.</em></b></div></section><section className="judge-takeaways"><div><div className="section-kicker">THE STORY IN 90 SECONDS</div><h2>One team.<br /><em>Four accountable outcomes.</em></h2></div><div className="judge-timeline">{[["00:00", "Routine help → verified"], ["00:20", "Important action → human"], ["00:40", "Human decision → verified"], ["01:00", "Mismatch → less authority"], ["01:20", "Unsafe instruction → blocked"]].map(([time, label], index) => <div key={time} className={index === 3 ? "critical" : ""}><time>{time}</time><span /><b>{label}</b></div>)}</div></section><section className="judge-watch"><div className="watch-panel"><div className="section-kicker">WHAT TO NOTICE</div><h2>Follow who<br /><em>decides what.</em></h2>{["A person or community has a real need.", "Strands helps the agent investigate and recommend.", "VOUCH checks trust, authority, risk, and evidence.", "Important decisions return to a person.", "Only the protected server changes shared records.", "A fresh read proves whether the help worked."].map((item) => <p key={item}><Check size={14} />{item}</p>)}</div><div className="impact-panel"><KnightAuthority level={session.trust.autonomy} size="small" state="verified" /><div className="section-kicker">WHY THIS MATTERS</div><h2>More capacity.<br />Human responsibility.<br /><em>No blind trust.</em></h2><p>VOUCH helps groups use capable agents without giving up control over actions that affect the people they serve.</p></div></section><section id="judges-demo"><EarnedLifecycleDemo session={session} scenarios={scenarios} runScenario={runScenario} mutateAction={mutateAction} resetSession={resetSession} loading={loading} /></section><section className="technical-snapshot"><div><div className="section-kicker">SOPHISTICATED UNDERNEATH</div><h2>Built with real<br /><em>agent boundaries.</em></h2></div><div className="tech-grid">{[...runtime, ["AGENT TOOLS", "Evidence inspection · recommendation"], ["VOUCH AUTHORITY ENGINE", "Fresh deterministic decision per request"], ["PEOPLE STAY IN CONTROL", "Hard limits return important choices"], ["AUDIT LAYER", "Evidence · outcome · authority provenance"]].map(([name, text]) => <div key={name}><Layers3 size={16} /><b>{name}</b><span>{text}</span></div>)}</div></section></div>;
}
...
function Architecture({ navigate }: { navigate: (v: View) => void }) {
  return <div className="architecture content-width">
    <div className="page-heading"><div><div className="eyebrow"><span className="eyebrow-line" />SIMPLE FOR PEOPLE · SOPHISTICATED UNDERNEATH</div><h1>The agent helps.<br /><em>VOUCH looks out.</em></h1><p>Every important request crosses the same server-side boundary so people do not have to trust the agent blindly.</p></div><div className="architecture-note"><LockKeyhole size={17} /><span><b>A fresh check every time</b><small>Model output and prior approval never grant future permission.</small></span></div></div>
